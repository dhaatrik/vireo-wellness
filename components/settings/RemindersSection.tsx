...
const ReminderItem = ({ reminder, onToggle, onDelete }: ReminderItemProps) => (
  <div className="p-4 border-b border-slate-800/50 flex items-center justify-between group py-3">
...
const AddReminderForm = ({ onAdd, onCancel }: AddReminderFormProps) => {
  ...
  const handleSubmit = () => {
    ...
    onAdd({ ...newReminder, isActive: true });
    ...
    <div className="p-4 border-b border-slate-800/50 bg-slate-800/20">
      <div className="space-y-3">
        ...
        <div className="flex justify-end gap-2 pt-2">
          <button onClick={onCancel} className="px-4 py-2 text-xs font-semibold text-slate-400 hover:text-white transition-colors">Cancel</button>
          <button onClick={handleSubmit} disabled={!newReminder.message.trim()} className="px-4 py-2 text-xs font-semibold bg-emerald-500 text-slate-950 rounded-lg hover:bg-emerald-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Add Reminder</button>
        </div>
      </div>
    </div>
  );
